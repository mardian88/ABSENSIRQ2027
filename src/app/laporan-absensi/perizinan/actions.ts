...
export async function updateStatusAlpa(idAbsensi: string, passwordAdmin: string, statusBaru: 'hadir' | 'izin'): Promise<{success: boolean, message: string}> {
  // Hardcoded password rqm as requested
  if (passwordAdmin !== 'rqm') {
    return { success: false, message: 'Password salah!' };
  }

  try {
    await db.update(absensi)
      .set({ statusKehadiran: statusBaru })
      .where(eq(absensi.id, idAbsensi));
    
    revalidatePath("/laporan-absensi/perizinan");
    return { success: true, message: `Status berhasil diubah menjadi ${statusBaru}` };
  } catch (error) {
