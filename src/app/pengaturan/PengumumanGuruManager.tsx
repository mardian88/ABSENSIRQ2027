...
import { getPengumumanGuru, tambahPengumumanGuru, updatePengumumanGuru, hapusPengumumanGuru } from "./actions";
...
export function PengumumanGuruManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (form.id) {
        await updatePengumumanGuru(form.id, form.judul, form.isi, form.isAktif);
        toast.success("Pengumuman diperbarui");
      } else {
        await tambahPengumumanGuru(form.judul, form.isi, form.isAktif);
  ...
  const handleToggleAktif = async (item: any) => {
    ...
    if (await showConfirm(`${item.isAktif ? 'Nonaktifkan' : 'Aktifkan'} Pengumuman?`, `Yakin ingin ${actionText} pengumuman ini?`, "Ya, Lanjutkan", item.isAktif)) {
      try {
        await updatePengumumanGuru(item.id, item.judul, item.isi, !item.isAktif);
  ...
  return (
    <Card className="mt-6 animate-in fade-in duration-300 slide-in-from-bottom-2">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Pengumuman Portal Guru</CardTitle>
          <CardDescription>Kelola informasi dan pengumuman untuk ditampilkan di Dashboard Portal Guru.</CardDescription>
        </div>
        {!isEditing && (
        ...
        {isEditing ? (
          <form onSubmit={handleSubmit} className="space-y-4 bg-slate-50 p-4 rounded-lg border">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="isAktif">Tampilkan di Portal Guru (Aktif)</Label>
            </div>
