...
export async function submitIzin(formData: FormData) {
  ...
  // --- VALIDASI IZIN GANDA ---
  // Cek apakah santri sudah memiliki izin yang overlap (aktif) di rentang tanggal ini
  ...
  // Loop through days from start to end and create an array for batch insert
  ...
  // Execute batch insert in a single query
  if (batchData.length > 0) {
    await db.insert(absensi).values(batchData);
  }
